...
export default function aclPlugin(schema, opts = {}) {
  ...
  schema.statics.checkResourceAcl = function(user, role, privilege = 'R', asserts = []) {
    return acl.resource(user, role, privilege, asserts)
  }

  schema.statics.filterByAcl = function(user, resource, role, privilege = 'R', asserts = []) {
    return acl.filter(user, resource, role, privilege, asserts)
  }
